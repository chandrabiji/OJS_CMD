...
public class OrderMapper implements ResultSetMapper<Order> {
  ...
  public  final Order map(final int idx, final ResultSet rs, final StatementContext ctx) throws SQLException {
       ...
       * @return Orderr
       */
    return new Order(rs.getInt("ORDER_ID"), rs.getInt("FOOD_ID"), rs.getInt("CUSTOMER_ID"), rs.getInt("VENDOR_ID"),
      rs.getInt("NO_OF_ITEMS"), rs.getString("ORDER_DATE_TIME"), rs.getDouble("AMOUNT_TO_BE_PAID"),
      rs.getInt("TOKEN_NUMBER"), rs.getString("STATUS"), rs.getString("COMMENTS"));

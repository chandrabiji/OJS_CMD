...
public class VendorMapper implements ResultSetMapper<Vendor> {
  ...
  public  final Vendor map(final int idx, final ResultSet rs, final StatementContext ctx) throws SQLException {
       ...
       * @return Vendor
       */
    return new Vendor(rs.getInt("vendor_id"), rs.getString("vendor_name"), rs.getString("vendor_email"),
      rs.getString("vendor_contactno"), rs.getString("vendor_address"), rs.getString("vendor_password"));
  }
}
